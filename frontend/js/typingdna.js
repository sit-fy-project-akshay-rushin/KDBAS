...
function TypingDNA() {
  if (TypingDNA.initialized != true) {
    ...
    TypingDNA.deviceOrientationHandler = function(e) {
      TypingDNA.kpLastPitch = Math.round(10 * e.beta);
      var roll = Math.round(10 * e.gamma);
      if (Math.abs(TypingDNA.kpLastPitch) > 900) {
        roll = ((roll < 0) ? 1800 : -1800) + roll;
      }
      TypingDNA.kpLastRoll = roll
      if (!TypingDNA.hasDeviceOrientation) {
    ...
    if (TypingDNA.isMobile() === 1) {
      ...
      if (window.DeviceOrientationEvent != undefined) {
        window.addEventListener('deviceorientation', TypingDNA.deviceOrientationHandler);
      }
    } else {
    ...
    TypingDNA.kpGetAll = function() {
      if (TypingDNA.kpAccZ.length < 2) {
        var returnVal = (TypingDNA.hasDeviceMotion && TypingDNA.hasDeviceOrientation) ? 0 : TypingDNA.lastPressTime;
        var returnMotion = [0, 0, 0, 0, 0, 0, TypingDNA.kpLastPitch, TypingDNA.kpLastRoll];
        ...
        var returnVal = lastKpTime;
        var returnMotion = [TypingDNA.kpLastAccX, TypingDNA.kpLastAccY, TypingDNA.kpAccZ.pop(), TypingDNA.kpX.pop(), TypingDNA.kpY.pop(), TypingDNA.kpLastZ, TypingDNA.kpLastPitch, TypingDNA.kpLastRoll];
        TypingDNA.kpX = [];
        TypingDNA.kpY = [];
        TypingDNA.kpAccZ = [];
        TypingDNA.kpTimes = [];
